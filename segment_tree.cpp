...
// rangeSet 和 rangAdd 的区别主要在于 rangeUpdate 和 maintain
...
private:
    ...
    void _rangeSet(int l, int r, int val, int cl, int cr, int p) {
        // [l, r] 为修改区间，val 为被修改元素的变化量
        // [cl, cr] 为当前结点包含的区间
        // p 为当前结点的编号
        if (l <= cl && cr <= r) {
        ...
        int m = cl + (cr -cl) / 2;
        maintain(cl, cr, p);
        if (l <= m) _rangeSet(l, r, val, cl, m, p * 2);
        if (r > m) _rangeSet(l, r, val, m + 1, cr, p * 2 + 1);
        tree[p] = tree[p * 2] + tree[p * 2 + 1];
    }
...
public:
    ...
    int rangeSum(int l, int r) {return _rangeSum(l, r, 0, end, root); }
    void rangeSet(int l, int r, int val) { _rangeSet(l, r, val, 0, end, root); }
